...
export class PostStats {
    ...
    */
    ...
    디비에 실제로 생성되는 컬럼은 이 테이블의 주키인 post_id 컬럼이다. 그런데 여기서 주키는 동시에 외래키 역할을 해야하기 때문에,
    ...
    @JoinColumn({ name: 'post_id' }) : PostStats 테이블에 'post_id'라는 이름의 외래 키 컬럼 생성
    */
    @OneToOne(() => Post, { onDelete: 'CASCADE' })
    @JoinColumn({ name: 'post_id' }) // "Post의 id를 참조하는 PostStats 테이블의 컬럼명을 post_id로 하겠다"라는 의미
    post: Post;
       /*
        결과적으로 생성되는 테이블 구조:
        CREATE TABLE post_stats (
            post_id integer PRIMARY KEY,  -- 동시에 외래 키
            ... other columns ...,
            CONSTRAINT fk_post 
                FOREIGN KEY (post_id) 
                REFERENCES post(id) 
